...
def extend(inputt, extend_number):
    ...
    for a in inputt:
        x = np.repeat(a.reshape(1, np.shape(inputt)[-1]), extend_number, axis=0)  # np.shape(inputt)[-1] == 1
            ...
            for iterr, i in enumerate(tqdm(dataloader[phase])):
                ...
                with torch.set_grad_enabled(phase == 'train' or phase == 'val'):
                    ...
                    for batch in range(len(pairs_info)):
                        ...
                        temp_scores = extend_object(np.array(objects_score).reshape(len(objects_score), 1),
                                                    # temp score=[objects_scores*people_number,pair number]
                    ...
                    #### Applying LIS = Low grade Instance Suppressive Function#######
                    persons_score_extended = LIS(persons_score_extended, 8.3, 12,
                                                 10)
                    ...
                    lossf = torch.sum(loss_com_combine(
                        sigmoid(outputs) * sigmoid(outputs_combine) * sigmoid(
                            outputs_single) * hum_obj_mask * sigmoid(
                ...
                if phase == 'test':
                    if (epoch + 1) % saving_epoch == 0 or infr == 't':
                        all_scores = filtering(predicted_HOI, true, persons_np_extended[1:],
