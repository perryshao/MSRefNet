...
from dataset.dataloader_vcoco import Rescale, ToTensor, vcoco_Dataset, vcoco_collate
from utils.train_test import train_test
import models.model as rr
import random
...
parser.add_argument('-e', '--number_of_epochs', type=int, required=False, default=80,
...
parser.add_argument('-sa', '--saving_epoch', type=int, required=False, default=20,
                    help='In which epoch to save e.g. 10 means the model would be saved every tenth epoch while training')
parser.add_argument('-fw', '--first_word', type=str, required=False, default='new_test',
                    help='Name of the folder in which you want to save')
parser.add_argument('-ba', '--batch_size', type=int, required=False, default=2, help='Batch size')
...
for name, p in res.named_parameters():
    if name.split('.')[0] == 'Conv_pretrain':
        ...
        if name.split('.')[0] == 'conv_sp_map' or name.split('.')[0] == 'spmap_up' or name.split('.')[0] == 'posemap_up' or name.split('.')[0] == 'conv_single_map':
            spmap.append(p)
        else:
            trainables.append(p)
