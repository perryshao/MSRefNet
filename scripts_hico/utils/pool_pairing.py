...
def get_pool_loc(ims, image_id, flag_, size=(7, 7), spatial_scale=1, batch_size=1):
    ...
    avg_pool_64 = nn.AdaptiveAvgPool2d((64, 64))
    for batch in range(batch_size):
        ...
        # distance=
        # distance = helpers_pre.get_distance(this_image, flag)
        # distance_out.append(torch.tensor(distance).cuda().float())
        ...
        roi_pose = pose_pers * pose_scale        # human 在pose map中的位置
    ...
    return torch.stack(pers_out), torch.stack(objs_out), spatial_locs, torch.cat(union_box_out), torch.cat(one_hot_out), torch.cat(pose_box)    #, torch.cat(distance_out)
...
def pairing(pers, objs, context, spatial_locs, pairs_info, object_one_hot):
    ...
    for batch in range(len(pairs_info)):
        ...
        batch_pers, batch_objs, batch_obj_one_hot = pers[start_p:start_p + this_batch_per], objs[start_o:start_o + this_batch_obj], object_one_hot[start_o:start_o+this_batch_obj_one_hot]  # 当前样本 p-feature o-feature [number,1024]
        ...
        for ind_p, i in enumerate(batch_pers):
            ...
            for ind_o, j in enumerate(batch_objs):
                ...
                pers_objs.append(torch.cat([i, j, sp_features], 0))      # every pair shape =[1,1024+1024+4] cat(,0)竖着拼，cat(,1)横着拼
                this_batch_pair_onehot.append(batch_obj_one_hot[ind_o])
                # pers_objs.append(torch.cat([i,j],0))


        pers_objs_batch = torch.stack(pers_objs)  # [pair number,2052]

        pairs_out.append(torch.cat([pers_objs_batch, batch_context.repeat(pers_objs_batch.size()[0], 1)], 1))    # [pair number,3076] 把context续接
        pers_out.append(batch_pers)                  # 和输入不变
        objs_out.append(batch_objs)                  # [o_number,1024]
        pairs_onehot_out.append(torch.stack(this_batch_pair_onehot))
        # context_out.append(torch.cat([batch_context.repeat(pers_objs_batch.size()[0], 1)], 1))
